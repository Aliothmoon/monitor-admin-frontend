        ...
        <a-row :gutter="16" class="monitor-stats">
          ...
          <a-col :span="6">
            <a-card class="stat-card warning-card">
              ...
              <div class="stat-footer">
                <a-tag color="red" size="small">需要处理: {{ stats.pendingAnomalies }}</a-tag>
              </div>
            </a-card>
          </a-col>
          <a-col :span="6">
            <a-card class="stat-card">
              ...
              <div class="stat-footer">
                <a-tag color="green" size="small">在线率: {{ Math.round(stats.onlineProctors/stats.totalProctors*100) }}%</a-tag>
              </div>
            </a-card>
          </a-col>
          <a-col :span="6">
            <a-card class="stat-card">
              ...
              <div class="stat-footer">
                <a-link>查看考生分布 <icon-right /></a-link>
              </div>
            </a-card>
          </a-col>
        </a-row>
                ...
                <a-list-item v-for="(exam, index) in examSchedule" :key="index">
                  <a-space direction="vertical" fill style="width: 100%">
                    ...
                    <a-row justify="space-between">
                      <a-col>
                        <a-space>
                          <icon-user /> {{ exam.proctor }}
                        </a-space>
                      </a-col>
                      <a-col>
                        <a-space>
                          <icon-user-group /> {{ exam.candidateCount }} 名考生
                          <a-button v-if="exam.status === '进行中'" type="text" size="mini">
                            监控
                          </a-button>
                        </a-space>
                      </a-col>
              ...
              <div v-else class="camera-grid">
                ...
                >
                  ...
                  <div class="camera-info">
                    <span>{{ candidate.name }} <a-tag :color="candidate.cameraStatus === '正常' ? 'green' : 'red'" size="small">{{ candidate.cameraStatus }}</a-tag></span>
                  </div>
                </div>
              </div>
              <template #extra>
...
import { getMonitorStats, getExamSchedule, getSystemNotifications, getCandidateLiveData } from './index';
...
const getStatusColor = (status: string) => {
  switch(status) {
    ...
    case '即将开始': return 'blue';
    case '已结束': return 'gray';
    case '已排期': return 'purple';
    default: return 'gray';
  }
};
...
const fetchData = async () => {
  try {
    ...
    // 并行获取所有数据
    const [statsRes, examRes, notificationsRes, liveDataRes] = await Promise.all([
      getMonitorStats(),
      getExamSchedule(),
...
.monitor-stats {
  .stat-card {
    ...
    .stat-icon {
      ...
      &.blue-icon { color: rgb(var(--arcoblue-6)); }
      &.red-icon { color: rgb(var(--red-6)); }
      &.green-icon { color: rgb(var(--green-6)); }
      &.yellow-icon { color: rgb(var(--orange-6)); }
    }
...
.exam-schedule-card, .live-monitoring-card, .activity-card, .quick-actions-card {
  height: 100%;
